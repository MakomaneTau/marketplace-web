...
export default function SellerDashboardPage() {
  ...
  return <div className="space-y-8"><SectionHeading eyebrow="Seller overview" title={data ? data.shop.name : "Seller dashboard"} description="Here is what is happening with your shop today." action={<Link href="/seller/products/new" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white"><Plus className="h-4 w-4"/>Add a product</Link>}/>{data && <><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Active listings" value={String(data.products.active)} helper={`${data.products.total} total`} icon={Package}/><StatCard label="Open orders" value={String(data.orders.open)} helper={`${data.orders.total} total`} icon={ShoppingBag}/><StatCard label="Product views" value={String(data.products.views)} icon={Eye}/><StatCard label="Unread messages" value={String(data.unread_messages)} helper={`${data.unread_notifications} notifications`} icon={MessageSquare}/></section><section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="font-bold">Recent orders</h2><div className="mt-4 divide-y">{data.orders.recent.map((order) => <div key={order.id} className="flex justify-between py-3"><div><p className="font-semibold">Order {order.id.slice(0, 8)}</p><p className="text-xs capitalize text-slate-500">{order.status.replaceAll("_", " ")}</p></div><p className="font-bold">R{Number(order.total_amount).toFixed(2)}</p></div>)}</div></section></>}{!data && !error && <p className="text-slate-500">Loading dashboard...</p>}{error && <p role="alert" className="text-red-700">{error}</p>}</div>;
}
